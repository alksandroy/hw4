...
class BinarySearchTree {
   public:
    ...
    virtual void insert(
        const std::pair<const Key, Value>& keyValuePair);  // TODO
   ...
   protected:
    ...
    static void deleteSubtree(Node<Key, Value>* current);             // TODO
    static int minDeep(Node<Key, Value>* current);                    // TODO
    static int maxDeep(Node<Key, Value>* current);                    // TODO
    ...
    while (curr != nullptr) {
        ...
        if (curr->getKey() > key) {
            if (curr->getLeft() != nullptr) {
                ...
                // curr->getLeft() = new Node<Key, Value>(key, value, curr);
                Node<Key, Value>* temp = new Node<Key, Value>(key, value, curr);
                curr->setLeft(temp);
                break;
            }
            ...
            if (curr->getRight() != nullptr) {
                ...
                // curr->getRight() = new Node<Key, Vaue>(key, value, curr);
                Node<Key, Value>* temp = new Node<Key, Value>(key, value, curr);
                curr->setRight(temp);
                break;
            }
...
void BinarySearchTree<Key, Value>::remove(const Key& key) {
    ...
    if (curr->getLeft() == nullptr || curr->getRight() == nullptr) {
        Node<Key, Value>* replacer =
            curr->getLeft() == nullptr ? curr->getRight() : curr->getLeft();

        if (par != nullptr) {
            if (par->getKey() > key) {
                par->setLeft(replacer);
                // par->getLeft() = replacer;
            } else {
                par->setRight(replacer);
                // par->getRight() = replacer;
            }
            if (replacer != nullptr) {
                replacer->setParent(par);
                // replacer->getParent() = par;}
            }
        } else {
            root_ = replacer;
            if (replacer != nullptr)
                replacer->setParent(nullptr);
        }
        delete curr;
        return;
    }
    Node<Key, Value>* prede = predecessor(curr);
    nodeSwap(curr, prede);
    Node<Key, Value>* childOfCurr =
        curr->getLeft() == nullptr ? curr->getRight() : curr->getLeft();

    if (childOfCurr != nullptr)
        childOfCurr->setParent(curr->getParent());

    if (curr->getParent()->getLeft() == curr)
        curr->getParent()->setLeft(childOfCurr);
    else
        curr->getParent()->setRight(childOfCurr);
...
template <class Key, class Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::predecessor(
...
template <class Key, class Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::successor(
    Node<Key, Value>* current) {
    // TODO
    if (current == nullptr)
        return current;
    ...
    //     return current->getParent();
    // }
    // return successor(current->getParent());
}
...
template <typename Key, typename Value>
int BinarySearchTree<Key, Value>::minDeep(Node<Key, Value>* current) {
    if (current == nullptr)
        return 0;
    int a = minDeep(current->getLeft());
    int b = minDeep(current->getRight());
    return a < b ? a + 1 : b + 1;
}

template <typename Key, typename Value>
int BinarySearchTree<Key, Value>::maxDeep(Node<Key, Value>* current) {
    if (current == nullptr)
        return 0;
    int a = maxDeep(current->getLeft());
    int b = maxDeep(current->getRight());
    return a > b ? a + 1 : b + 1;
}
...
bool BinarySearchTree<Key, Value>::isBalanced() const {
    // TODO
    return maxDeep(root_) - minDeep(root_) <= 1;
}
